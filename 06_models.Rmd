...
## Generate predictions to graph, step by step
...
### Grouped PCA in a single pipeline
...
gss_sm <- gss_sm %>% 
  mutate(polviews_m = relevel(polviews, ref = "Moderate"))
...
```{r 06-models-33}
p <- ggplot(data = bo_gg, aes(y = reorder(factor, AME),
...
p <- ggplot(data = pv_cp, aes(y = reorder(xvals, yvals),
                              x = yvals, xmin = lower, xmax = upper))
